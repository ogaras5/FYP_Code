...
dataset expanded with multiple single augmentations.
"""
...
parser.add_argument('--augmentations', type=str, nargs='+', default=['benchmark'],
...
parser.add_argument('-f', '--filename', type=str, default='benchmark', help='Start of csv filename (default:benchmark)')
...
def main():
    ...
    for augment in args.augmentations:
        if ''.join(filter(lambda x: x.isalpha(), augment)) != 'benchmark':
            # Add augmentation to pipeline (see utils/data_helpers.py for code)
            p = create_augmentation_pipeline(augment, args.probability, args.value, args.value, args.magnitude)
    ...
    def train_model(model, criterion, optimizer, num_epochs=10):
        ...
        for epoch in range(args.start_epoch, args.start_epoch + num_epochs):
            ...
            # Save checkpoint
            checkpoint_filename = './checkpoints/{}/multiAugment/{}-{}-{}-{:03d}.pkl'.format(args.dataset,
            ...
            df.to_csv("./losses/{}/multiAugment/{}-{}-{}-tmp.csv".format(args.dataset, args.filename, args.sample_size, args.dataset))

            # Save details about time of training to tmp file
            time_elapsed = time.time() - since
            fp = open('./losses/{}/multiAugment/{}-{}-details-tmp.txt'.format(args.dataset, args.dataset, args.sample_size), 'w+')
        ...
        fp = open('./losses/{}/multiAugment/{}-{}-details.txt'.format(args.dataset, args.dataset, args.sample_size), 'a+')
        fp.write('\nResults for training {}:\n Start epoch {}, End epoch {}, Training time {:.0f}m {:.0f}s, Best Validation accuracy {:4f}%, Best Training accuracy {:4f}%'.format(args.filename,
    ...
    if args.start_epoch != 1:
        epoch = load_checkpoint(optimizer, model_res,
                            './checkpoints/{}/multiAugment/{}-{}-{}-{:03d}.pkl'.format(args.dataset,
    ...
    if args.start_epoch != 1:
        old_df = pd.read_csv('./losses/{}/multiAugment/{}-{}-{}.csv'.format(args.dataset, args.filename, args.sample_size, args.dataset))
    ...
    # Save to csv file
    df.to_csv('./losses/{}/multiAugment/{}-{}-{}.csv'.format(args.dataset, args.filename, args.sample_size, args.dataset))
