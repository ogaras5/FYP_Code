"""
Script to train sampled/full version of CIFAR-10/CIFAR-100 Dataset.
Can be used to train dataset expanded with combined augmentations.
"""
...
parser = argparse.ArgumentParser(description='PyTorch Sampled CIFAR10/CIFAR100 Training Pair Augmentations')
...
parser.add_argument('--augmentations', type=str, nargs='+', default=['erase'],
                    help='Type of augmentation to apply to the dataset (default: erase)')
...
parser.add_argument('-f', '--filename', type=str, default='erase', help='Start of csv filename (default:erase)')
...
def main():
    ...
    p = create_augmentation_pipeline(args.augmentations, args.probability, args.value, args.value, args.magnitude)
    ...
    # Combine original dataset with augmented_set
    ...
    def train_model(model, criterion, optimizer, num_epochs=10):
        ...
        for epoch in range(args.start_epoch, args.start_epoch + num_epochs):
            ...
            # Save checkpoint
            checkpoint_filename = './checkpoints/{}/pairAugment/{}-{}-{}-{:03d}.pkl'.format(args.dataset,
                                    args.filename, args.sample_size, args.dataset, epoch)
            ...
            df.to_csv("./losses/{}/pairAugment/{}-{}-{}-tmp.csv".format(args.dataset, args.filename, args.sample_size, args.dataset))

            # Save details about time of training to tmp file
            time_elapsed = time.time() - since
            fp = open('./losses/{}/pairAugment/{}-{}-details-tmp.txt'.format(args.dataset, args.dataset, args.sample_size), 'w+')
            fp.write('\nResults for training {}:\n Start epoch {}, End epoch {}, Training time {:.0f}m {:.0f}s, Best Validation accuracy {:4f}%, Best Training accuracy {:4f}%'.format(args.filename,
        ...
        fp = open('./losses/{}/pairAugment/{}-{}-details.txt'.format(args.dataset, args.dataset, args.sample_size), 'a+')
        fp.write('\nResults for training {}:\n Start epoch {}, End epoch {}, Training time {:.0f}m {:.0f}s, Best Validation accuracy {:4f}%, Best Training accuracy {:4f}%'.format(args.filename,
    ...
    if args.start_epoch != 1:
        epoch = load_checkpoint(optimizer, model_res,
                            './checkpoints/{}/pairAugment/{}-{}-{}-{:03d}.pkl'.format(args.dataset,
                            args.filename, args.sample_size, args.dataset, args.start_epoch-1))
    ...
    if args.start_epoch != 1:
        old_df = pd.read_csv('./losses/{}/pairAugment/{}-{}-{}.csv'.format(args.dataset, args.filename, args.sample_size, args.dataset))
    ...
    # Save to csv file
    df.to_csv('./losses/{}/pairAugment/{}-{}-{}.csv'.format(args.dataset, args.filename, args.sample_size, args.dataset))
